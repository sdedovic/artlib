...
__device__ float3 calcNoised2(float2 pos, float random) {
  ...
  float2 derivatives = ga + u.x * (gb - ga) + u.y * (gc - ga) +
                       u.x * u.y * (ga - gb - gc + gd) +
                       du * (make_float2(u.y, u.x) * (va - vb - vc + vd) +
                             make_float2(vb, vc) - va);
  return make_float3(value, derivatives.x, derivatives.y);
}

__global__ void noise2(float *buffer, float *rngBuffer, int2 resolution,
  ...
  float2 pos =
      (make_float2(x, y) / make_float2(resolution.x, resolution.y)) * scale +
      offset;
  buffer[idx] = calcNoised2(pos, random).x;
}

__global__ void noised2(float *buffer, float *rngBuffer, int2 resolution,
                        float2 scale, float2 offset) {
  int x = threadIdx.x + blockIdx.x * blockDim.x;
  int y = threadIdx.y + blockIdx.y * blockDim.y;
  if (x >= resolution.x || y >= resolution.y)
